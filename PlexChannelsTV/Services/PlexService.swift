            ...
            while true {
                do {
                    ...
                    if page.count < batchSize {
                        ...
                        // For movies and episodes, continue fetching even if page is smaller than batch size
                        // This handles cases where Plex returns 399 items instead of 400
                        if (mediaType == .movie || mediaType == .episode) && page.count > 0 {
                            AppLoggers.net.info("event=plexService.fetchLibraryItems.continue libraryType=\(mediaType.rawValue) pageCount=\(page.count) batchSize=\(batchSize)")
                            start += page.count
                            continue
                        }
                        break
